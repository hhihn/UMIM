...
fig = plt.figure(figsize
                 =(20, 32))
grid = plt.GridSpec(6, 5, hspace=0.35)
upper_ax = fig.add_subplot(grid[0, :])
lower_axes = [fig.add_subplot(grid[i+1, i:]) for i in range(5)]
...
# hvcl_file_4 = "4_expert_no_replay_16units_10eval_1M_dloeep_nr.npy"
...
###
dense_file = "umim_eval_baselines.npy"
...
sns.lineplot(x=xx, y=dense_mean, linewidth=3, label="Naive Sequential Training", ax=upper_ax, color=colors[4])
...
# plt.suptitle("Continual Reinforcement Learning Benchmark")
...
for li, loa in enumerate(lower_axes):
    ...
    if li == 4:
        hvcl4_data[li] = hvcl4_data[li] * 10
        hvcl4_std[li] = hvcl4_std[li] * 3
        ucl_data[li] = ucl_data[li] * 10
        ucl_data_std[li] = ucl_data_std[li] * 3

    x = np.arange(start=start_idx, step=1, stop=len(hvcl4_data[li]))
    y = hvcl4_data[li][start_idx:]
    std = hvcl4_std[li][start_idx:]
    sns.lineplot(x=x, y=y, ax=loa, linewidth=3, label=label2, color=colors[3])
    loa.fill_between(x, y - std, y + std, alpha=0.25, color=colors[3])

    x = np.arange(start=start_idx, step=1, stop=len(ewc_data[li]))
    y = ewc_data[li][start_idx:]
    sns.lineplot(x=x, y=y, ax=loa, linewidth=3, label=label5, color=colors[1])
    loa.fill_between(x, y - ewc_data_std[li][start_idx:], y + ewc_data_std[li][start_idx:], alpha=0.25, color=colors[1])

    x = np.arange(start=start_idx, step=1, stop=len(ucl_data[li]))
    y = ucl_data[li][start_idx:]
    sns.lineplot(x=x, y=y, ax=loa, linewidth=3, label=label3, color=colors[2])
    loa.fill_between(x, y - ucl_data_std[li][start_idx:], y + ucl_data_std[li][start_idx:], alpha=0.25, color=colors[2])

    loa.set_xlim([start_idx, 500])
    loa.set_xticks(ticks=np.arange(start=start_idx, step=100, stop=600))
    loa.set_xticklabels(np.arange(start=li, step=1, stop=6))
