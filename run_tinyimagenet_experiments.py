...
from build_imagenet import extract_classes_imagenet
...
def run_experiment(params, x_train, y_train, x_test, y_test):
    ...
    for task_id, classes in enumerate(pairs):
        ...
        orig_curr_x_train, orig_curr_y_train = extract_classes_imagenet(classes, num_classes=num_classes)
        ...
        centr_ls = [0.0]
        eval_accuracies = [0.0]
        curr_x_test, curr_y_test = extract_classes_imagenet(classes, num_classes=num_classes, mode="val")
...
def mean_experiment(params):
    ...
    dump(file=open("tinyimagenet_small_data.pkl", "wb"), obj=[acc, bwt, avg_bwt, rem, pwt, tmi, acc_mats, ttgms, acc_mats, ttgms])
    acc, bwt, avg_bwt, rem, pwt, tmi, acc_mats, ttgms, acc_mats, ttgms = load(open("tinyimagenet_small_data.pkl", "rb"))
