        ...
        for x, xxp, xxn, xxx, y in train_dataset:
            ...
            if not first_run and len(dispatched_inputs) > len(models):
                ...
                if len(unmatched_samples) >= new_model_threshold and len(models) < max_num_models:
                    ...
                    if verbose:
                        print("\033[92m=" * 50)
                        print("| creating new modeel %d with %d samples \t|" % (num_models, len(unmatched_samples)))
                        print("=" * 50)
...
def mean_experiment(params):
    ...
    dump(file=open("cifar100_small_data.pkl", "wb"), obj=[acc, bwt, avg_bwt, rem, pwt, tmi, acc_mats, ttgms, acc_mats, ttgms])
    acc, bwt, avg_bwt, rem, pwt, tmi, acc_mats, ttgms, acc_mats, ttgms = load(open("cifar100_small_data.pkl", "rb"))
